...
import { MemoryMandateStateStore } from "./mandate-state.js";
...
export async function authorize({ catalog, selectedProduct, checkout, openMandate, paymentMandate, secret, ledger, now = Date.now() }) {
  ...
  const issuance = await ledger.createMandateState({
    mandateId: paymentMandate.id,
    checkoutHash: paymentMandate.checkoutHash,
    issuedAt: paymentMandate.issuedAt,
    expiresAt: paymentMandate.expiresAt,
  }, now);
  if (issuance.state === "ISSUED") {
    const backend = ledger.kind === "upstash" ? "Upstash Redis" : "local test state";
    evidence.push(trace("MANDATE_ISSUED", "verified", `Single-use state persisted in ${backend}; TTL is capped by the signed expiry.`));
  }
  const reservation = await ledger.compareAndTransition(paymentMandate.id, "ISSUED", "RESERVED", {
    now,
    checkoutHash: paymentMandate.checkoutHash,
    metadata: { reservedAt: now, lastEvent: "MANDATE_RESERVED" },
  });
  if (!reservation.ok) {
    const code = reservation.reason === "CHECKOUT_HASH_MISMATCH" ? "CHECKOUT_HASH_MISMATCH" : "MANDATE_ALREADY_CONSUMED";
    const detail = `Payment Mandate state is ${reservation.state}; atomic reservation denied and no Razorpay order was created.`;
    return {
      decision: "blocked",
      code,
      detail,
      trace: [...evidence, trace("REPLAY_BLOCKED", "blocked", detail)],
    };
  }
  evidence.push(trace("MANDATE_RESERVED", "verified", "ISSUED → RESERVED atomically before Razorpay order creation."));
...
export async function runMandate(openMandate, { now = Date.now(), secret, ledger = new MemoryMandateStateStore(), catalog = baseCatalog } = {}) {
  if (!secret) throw new Error("A signing secret is required.");
  const openFailure = openMandateFailure(openMandate, secret, now);
  if (openFailure) {
    return {
  ...
  return { catalog: catalogSnapshot, selectedProduct, checkout, openMandate, paymentMandate, result: await authorize(input) };
}
...
export async function runSuite(options = {}) {
  const runs = await Promise.all(scenarios.map(async (scenario) => {
    const run = await runScenario(scenario.id, options);
    return {
      ...scenario,
