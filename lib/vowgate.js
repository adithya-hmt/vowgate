...
export function verifyOpenMandate(mandate, secret) {
  if (!mandate || typeof mandate !== "object" || !secret) return false;
...
export function verifyOrderEvidence(evidence, secret) {
  if (!evidence || typeof evidence !== "object" || !secret) return false;
...
export function createCheckout(product, catalog, mandate) {
  const quantity = mandate.constraints.quantity;
  const checkout = {
    id: `checkout_${mandate.id.slice(-12)}`,
    merchantId: catalog.merchantId,
    catalogVersion: catalog.version,
    items: [{ sku: product.sku, name: product.name, quantity, unitAmount: product.price }],
...
export class RedemptionLedger {
  ...
  release(paymentMandateId) {
    this.#consumed.delete(paymentMandateId);
  }
}
...
export function runMandate(openMandate, { now = Date.now(), secret, ledger = new RedemptionLedger() } = {}) {
  if (!secret) throw new Error("A signing secret is required.");
  if (!verifyOpenMandate(openMandate, secret)) {
    return {
      openMandate,
      result: {
        decision: "blocked",
        code: "INVALID_SIGNATURE",
        detail: "Open mandate signature is invalid.",
        trace: [trace("POLICY GATE", "blocked", "Open mandate signature is invalid.")],
      },
    };
  }

  const catalog = structuredClone(baseCatalog);
  const intent = { ...structuredClone(openMandate.constraints) };
  const selectedProduct = chooseProduct(intent, catalog);
  if (!selectedProduct) {
    return {
      intent,
      catalog,
      ...
      result: {
        ...
        detail: "No available product satisfies every approved constraint.",
        trace: [
          trace("INTENT", "verified", "Customer constraints recovered from the signed mandate."),
          trace("POLICY GATE", "blocked", "No available product satisfies every approved constraint."),
        ],
      },
    };
  }
  ...
  const paymentMandate = createPaymentMandate(checkout, openMandate, { now, id: openMandate.id });
  const input = { intent, catalog, selectedProduct, checkout, openMandate, paymentMandate, secret, ledger, now };
  return { intent, catalog, selectedProduct, checkout, openMandate, paymentMandate, result: authorize(input) };
}
